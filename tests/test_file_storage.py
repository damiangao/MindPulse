...
class TestFileStorage:
    def test_save_file_creates_directory(self, tmp_path):
        ...
        result = save_file(content, "user-abc", "chat-123", "test.txt", str(tmp_path))
    ...
    def test_save_file_returns_relative_path(self, tmp_path):
        ...
        result = save_file(b"test", "user-abc", "chat-123", "file.txt", str(tmp_path))
        assert result == "user-abc/chat-123/file.txt"

    def test_get_file_path_constructs_full_path(self, tmp_path):
        from server.file_storage import get_file_path

        path = get_file_path("user-abc/chat-123/file.txt", str(tmp_path))
        assert str(path) == str(tmp_path / "user-abc" / "chat-123" / "file.txt")
